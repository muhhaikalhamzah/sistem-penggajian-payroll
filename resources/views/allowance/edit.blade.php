<x-app>
    ...
    <div class="card shadow-lg p-3">
        <form action="{{ route('allowance.update', $allowance) }}" method="post" class="form">
            @csrf
            @method('put')
            <div class="mb-3">
                ...
                <select class="form-select select2-default @error('employee_id') is-invalid @enderror" id="employee_id" name="employee_id" required>
                    ...
                    @foreach ($employees as $emp)
                        <option value="{{ $emp->id }}" @selected(old('employee_id', $allowance->employee_id) == $emp->id)>{{ $emp->first_name }} {{ $emp->last_name }} ({{ $emp->employee_number }})</option>
                    @endforeach
                </select>
            ...
            <div class="mb-3">
                ...
                <input class="form-control @error('name') is-invalid @enderror" type="text" id="name"
                    name="name" required value="{{ old('name', $allowance->name) }}">
            ...
            <div class="mb-3">
                ...
                <select class="form-select @error('type') is-invalid @enderror" id="type" name="type" required>
                    ...
                    <option value="Fixed" @selected(old('type', $allowance->type) == 'Fixed')>Fixed (Tetap bulanan)</option>
                    <option value="Variable" @selected(old('type', $allowance->type) == 'Variable')>Variable (Berdasarkan hari masuk, dll)</option>
                </select>
            ...
            <div class="mb-3">
                ...
                <input class="form-control @error('amount') is-invalid @enderror" type="number" id="amount"
                    name="amount" required value="{{ old('amount', $allowance->amount) }}" min="0">
