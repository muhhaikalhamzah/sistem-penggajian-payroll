        ...
        <form action="{{ route('employee.store') }}" method="post" class="form">
            ...
            <div class="row g-3 mb-3">
                <div class="col-md-6">
                    ...
                    <input class="form-control @error('employee_number') is-invalid @enderror" type="text" id="employee_number" name="employee_number" required value="{{ old('employee_number') }}">
                ...
                <div class="col-md-6">
                    ...
                    <input class="form-control @error('join_date') is-invalid @enderror" type="date" id="join_date" name="join_date" required value="{{ old('join_date') }}">
                    @error('join_date') <div class="invalid-feedback">{{ $message }}</div> @enderror
                </div>
                <div class="col-md-6">
                    <label for="first_name" class="form-label required">Nama Depan</label>
                    <input class="form-control @error('first_name') is-invalid @enderror" type="text" id="first_name" name="first_name" required value="{{ old('first_name') }}">
                    @error('first_name') <div class="invalid-feedback">{{ $message }}</div> @enderror
                </div>
                <div class="col-md-6">
                    <label for="last_name" class="form-label required">Nama Belakang</label>
                    <input class="form-control @error('last_name') is-invalid @enderror" type="text" id="last_name" name="last_name" required value="{{ old('last_name') }}">
                    @error('last_name') <div class="invalid-feedback">{{ $message }}</div> @enderror
                </div>
                <div class="col-md-4">
                    <label for="ptkp_status" class="form-label required">Status PTKP</label>
                    <input class="form-control @error('ptkp_status') is-invalid @enderror" type="text" id="ptkp_status" name="ptkp_status" required value="{{ old('ptkp_status') }}">
                ...
                <div class="col-md-4">
                    ...
                    <select class="form-select @error('position_id') is-invalid @enderror" id="position_id" name="position_id" required>
                        ...
                        @foreach ($positions as $pos)
                            <option value="{{ $pos->id }}" @selected(old('position_id') == $pos->id)>{{ $pos->title }}</option>
                        @endforeach
                    </select>
